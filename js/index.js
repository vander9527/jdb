$(document).ready(function() {
    ...
    function renderContent() {
        ...
        html += '</div>';
        html += '<button class="refresh-btn" id="refreshBtn"><span class="refresh-icon">↻</span> 刷新</button>';
        html += '</div>';
                ...
                html += `
                    <div class="media-item" data-id="${item.id}" data-type="${item.type || ''}">
                        ...
                        <div class="media-info">
                            <div class="media-title">${item.title}（${item.total}）</div>
                        </div>
                    </div>
                `;
            } else {
    ...
    function initRefreshButton() {
        $('#refreshBtn').on('click', function() {
            ...
            categories.forEach(cat => {
                if (cat.type === 'indexed') {
                    localStorage.removeItem(`${cat.id}_data`);
                    localStorage.removeItem(`${cat.id}_index`);
                }
            });
